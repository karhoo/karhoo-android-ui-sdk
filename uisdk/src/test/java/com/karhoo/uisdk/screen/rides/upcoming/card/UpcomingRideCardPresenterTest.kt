...
class UpcomingRideCardPresenterTest {
    ...
    fun setup() {
        ...
        whenever(
            context.resources.getQuantityString(
                R.plurals.kh_uisdk_minutes_plurals, TEST_TWO_MINUTES, TEST_TWO_MINUTES
            )
        ).thenReturn(
            String.format(
                ServiceCancellationExtTests.TEST_CANCELLATION_TEXT_BEFORE_PICKUP_MINUTES,
                TEST_TWO_MINUTES
            )
        )
        whenever(context.getString(R.string.kh_uisdk_quote_cancellation_before_driver_departure)).thenReturn(
            TEST_CANCELLATION_DRIVER_EN_ROUTE_TEXT
        )
        whenever(context.getString(R.string.kh_uisdk_quote_cancellation_before_pickup_start)).thenReturn(
            ServiceCancellationExtTests.TEST_CANCELLATION_TEXT_BEFORE_PICKUP_START
        )
        whenever(context.getString(R.string.kh_uisdk_quote_cancellation_before_pickup_ending)).thenReturn(
            ServiceCancellationExtTests.TEST_CANCELLATION_TEXT_BEFORE_PICKUP_END
        )
        whenever(context.getString(R.string.kh_uisdk_quote_cancellation_before_driver_departure)).thenReturn(
            TEST_CANCELLATION_DRIVER_EN_ROUTE_TEXT
        )
        whenever(context.getString(R.string.kh_uisdk_quote_cancellation_after_booking_ending)).thenReturn(
            ServiceCancellationExtTests.TEST_CANCELLATION_TEXT_AFTER_BOOKING_END
        )
    }
    ...
    fun `When we have a free cancellation SLA with before pickup, the asap cancellation text is visible`() {
        ...
        verify(view).setCancellationText(
            String.format(
                BookingQuotesPresenterTest.TEST_CANCELLATION_TEXT_ASAP,
                TEST_TWO_MINUTES
            )
        )
    }
    ...
    fun `When we have a free cancellation SLA with before pickup, the prebook cancellation text is visible`() {
        ...
        verify(view).setCancellationText(
            String.format(
                BookingQuotesPresenterTest.TEST_CANCELLATION_TEXT_PREBOOK,
                TEST_TWO_MINUTES
            )
        )
    }
    ...
    fun `scheduled date view binding handled by ScheduledDateViewBinder`() {
        ...
        presenter =
            UpcomingRideCardPresenter(view, trip, scheduledDateViewBinder, analytics, context)
    ...
    companion object {
        ...
        private const val DRIVER_NUMBER = "98834 123934"
        ...
        val CANCELLATION_AGREEMENT_BEFORE_DRIVER_EN_ROUTE = ServiceAgreements(
            ServiceCancellation(
                CANCELLATION_BEFORE_DRIVER_EN_ROUTE,
                TEST_TWO_MINUTES
            )
        )
        val CANCELLATION_AGREEMENT_ZERO_MINUTES =
            ServiceAgreements(ServiceCancellation(CANCELLATION_TIME_BEFORE_PICKUP, 0))
        val CANCELLATION_AGREEMENT_BEFORE_PICKUP = ServiceAgreements(
            ServiceCancellation(
        ...
        const val TEST_CANCELLATION_DRIVER_EN_ROUTE_TEXT =
            "Free cancellation until driver is en route"
    }
